    ...
    class TwoLayerDeepForest(BaseEstimator, ClassifierMixin):
        def fit(self, X, y):
            ...
            for model in self.second_layer:
                model.fit(first_feature_vector, y)
...
def with_bosk_framework(data: Tuple[np.ndarray, ...]):
    ...
    X = b.Input('X')()
    y = b.TargetInput('y')()
    et_1 = b.ETC(random_state=1)(X=X, y=y)
    rf_1 = b.RFC(random_state=2)(X=X, y=y)
    feature_vector = b.Concat(['X', 'et', 'rf'])(rf=rf_1, et=et_1, X=X)
    et_2 = b.ETC(random_state=3)(X=feature_vector, y=y)
    rf_2 = b.RFC(random_state=4)(X=feature_vector, y=y)
