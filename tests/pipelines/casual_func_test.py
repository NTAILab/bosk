...
class CasualFuncForestTest(BPT):
    ...
    def get_pipeline(self) -> BasePipeline:
        ...
        et_1 = b.ETC(random_state=self.random_state, n_estimators=self.n_trees)(X=X, y=y)
        concat_1 = b.Concat(['X', 'rf_1', 'et_1'])(X=X, rf_1=rf_1, et_1=et_1)
        rf_2 = b.RFC(random_state=self.random_state, n_estimators=self.n_trees)(X=concat_1, y=y)
        et_2 = b.ETC(random_state=self.random_state, n_estimators=self.n_trees)(X=concat_1, y=y)
        concat_2 = b.Concat(['X', 'rf_2', 'et_2'])(X=X, rf_2=rf_2, et_2=et_2)
        rf_3 = b.RFC(random_state=self.random_state, n_estimators=self.n_trees)(X=concat_2, y=y)
        et_3 = b.ETC(random_state=self.random_state, n_estimators=self.n_trees)(X=concat_2, y=y)
