...
class PipelineTestBase(ABC):
    ...
    def fit_test(self):
        _, fit_output = fit_pipeline(self.get_pipeline(), self.get_fit_data(), RecursiveExecutor, *self.get_fit_in_out())
        logging.info(f'Test "{self.__class__.__name__}" provided following outputs \
            throung the fit procedure: {list(fit_output.keys())}')

    def transform_test(self):
        fitted_pipeline, _ = fit_pipeline(self.get_pipeline(), self.get_fit_data(), RecursiveExecutor, *self.get_fit_in_out())
        data = self.get_transform_data()
        executor = RecursiveExecutor(fitted_pipeline,
            HandlingDescriptor.from_classes(Stage.TRANSFORM), *self.get_transform_in_out())
        tf_output = executor(data)
        logging.info(f'Test "{self.__class__.__name__}" provided following outputs \
            throung the transform procedure: {list(tf_output.keys())}')
